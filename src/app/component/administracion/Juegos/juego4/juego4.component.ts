...
export class Juego4Component implements OnInit {
  ...
  compararYPuntuar() {
    let puntuacionUsuario = this.formGrupos.get('puntuacion').value;
  
    if(this.puntuacionJuego === puntuacionUsuario) {
      this.guardarDatos();
    } else {
      this._snackBar.open('La puntuación ingresada no coincide con la puntuación del juego', 'ACEPTAR');
    }
  }
}
